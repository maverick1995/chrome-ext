...
// code to evaluate the function in context of web-page

function injectScript(source , arguments )
{
...
// receiving msg from mocks.js about the function
msg = {
	content : 'request' , 
	sender : 'inject.js'
}
...
chrome.runtime.sendMessage(  msg , function(response){
    ...
    var for_eval =  'fun = ' + response.content ; 
    eval(for_eval) ; 
    args = [] ; 
    args.push(response.param) ; 
    var ret = injectScript(fun , response.param) ; 
    sc= document.createElement('script') ; 
    sc.innerHTML =  '('+ response.content + ')(' + response.param.join() + ')'  ; 
    document.head.appendChild(sc) ; 
    document.head.removeChild(sc) ; 
    resp.content = ret , 
    console.log(ret) ; 
	console.log(response) ; 
    chrome.runtime.sendMessage(resp , function(respon){
        console.log(respon) ;
    });
}) ; 
